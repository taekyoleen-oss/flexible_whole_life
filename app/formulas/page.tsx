  ...
  return (
    <div className="mx-auto max-w-3xl space-y-8">
      <div>
        ...
        <p className="mt-1 text-sm text-navy/60">앱의 각 숫자 옆 &quot;?&quot; 버튼이 보여주는 내용을 한곳에 모았습니다. <FormulaText text="기호: x 가입연령, t 경과년, n 보장기간, m 납입기간, S_t 사망보험금 배수, C_t 축하금 배수, S_0 기준보험금, v = 1/(1+예정이율). ′는 납입면제를 반영한 납입 집단 기준입니다." /></p>
        ...
        <section key={g} id={g} className="space-y-4">
          ...
          {FORMULAS.filter((f) => f.group === g).map((f) => (
            <article key={f.id} id={f.id} className="rounded-lg border border-navy/10 bg-white p-4 shadow-sm">
              <h3 className="font-medium text-navy"><FormulaText text={f.title} /></h3>
              <FormulaText block className="mt-2" text={f.formula} />
              <p className="mt-2 text-sm"><FormulaText text={f.meaning} /></p>
              {f.vars.length > 0 && <dl className="mt-2 grid grid-cols-[auto_1fr] gap-x-3 gap-y-0.5 text-xs">{f.vars.map(([k, v]) => <div key={k} className="contents"><dt className="font-mono text-navy"><FormulaText text={k} /></dt><dd className="text-navy/70">{v}</dd></div>)}</dl>}
