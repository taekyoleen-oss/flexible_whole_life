...
const lines = [
  ...
  "",
  "기호: x 가입연령, t 경과년, n 보장기간(최종연령 − x), m 납입기간, S_t 사망보험금 배수, C_t 축하금 배수, S_0 기준보험금(원), v = 1/(1+예정이율). 계산기수(D, N, C)는 라딕스 100,000 기준이며 ′ 표시는 납입면제를 반영한 납입 집단 기준입니다.",
  "",
  "## 차례",
  ...groups.map((g) => `- ${g}: ${formulas.filter((f) => f.group === g).map((f) => f.title).join(" · ")}`),
  "",
];
for (const g of groups) {
  lines.push(`## ${g}`, "");
  for (const f of formulas.filter((x) => x.group === g)) {
    lines.push(`### ${f.title}`, "", "```", f.formula, "```", "", f.meaning, "");
    if (f.vars.length) { lines.push("| 기호 | 뜻 |", "|---|---|", ...f.vars.map(([k, v]) => `| ${k} | ${v} |`), ""); }
