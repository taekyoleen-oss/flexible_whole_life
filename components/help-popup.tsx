...
export function HelpPopup({ title, children, label = "근거" }: { title: string; children: ReactNode; label?: string }) {
  ...
  return (
    <>
      ...
      <dialog ref={dlg} className="m-auto w-[min(92vw,560px)] rounded-lg bg-white p-5 text-left shadow-xl backdrop:bg-navy/50"
        onClick={(e) => { e.stopPropagation(); if (e.target === e.currentTarget) e.currentTarget.close(); }}>
