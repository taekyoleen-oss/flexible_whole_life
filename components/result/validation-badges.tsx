...
export function ValidationBadges() {
  ...
  if (violations.length === 0) return <div className="rounded border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm text-emerald-800">✓ 설계 제약 E01~E08 통과 <FormulaHelp id="envelope" /></div>;
  return (
    <ul className="space-y-2">
      <li className="text-xs text-navy/60">설계 제약 <FormulaHelp id="envelope" /></li>
      {violations.map((v) => (
