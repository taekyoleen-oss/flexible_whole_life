...
export function ReserveTable() {
  ...
  return (
    <details className="rounded-lg border border-navy/10 bg-white p-4 shadow-sm">
      <summary className="cursor-pointer font-display text-lg text-navy">준비금·해약환급금 표{eff}</summary>
      <div className="mt-2 flex flex-wrap items-center justify-between gap-2 text-xs text-navy/60">
        <span>경과년별 연말 준비금(적용·표준), 납입 누계, 해약환급금·환급률. 가정 {r.meta.assumptionId} · 기준보험금 {won(state.S0)}</span>
          ...
          <tbody>
            {rows.map((row) => (
              <tr key={row.t} className={`border-t border-navy/10 font-mono ${row.t === state.payYears ? "bg-sky/10" : ""}`}>
                ...
                <td className="text-right">{won(row.cash)}</td><td className="text-right">{pct(row.rate)}</td>
              </tr>
            ))}
          </tbody>
        </table>
