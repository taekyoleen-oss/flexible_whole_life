...
export function presetNeeds(id: PresetId, p: Profile, a: AssumptionSet, table: RateTable, n: number, env: EnvelopeParams): PresetNeeds | null {
  const np = a.needs;
  const preNeed = needs({ age: p.age, income: p.income, liquidAssets: p.liquidAssets, debt: p.debt, childrenAges: p.childrenAges, existingCover: p.groupCover + p.termCover }, np).needs;
  ...
  switch (id) {
    ...
    case "child": {
      ...
      const c = childNeedCurve({ income: p.income, childrenAges: p.childrenAges }, np, n);
      const youngest = Math.min(...p.childrenAges), tInd = Math.max(0, np.independenceAge - youngest);
      const living0 = c[0] - p.childrenAges.length * np.educationPerChild - np.finalExpense;
      return {
        available: true, target: c.map((v) => v / c[0]), baseNeed: c[0], proposedS0: roundS0(c[0] - p.liquidAssets - p.groupCover - p.termCover),
        figures: [
          [`유족 생활비 현가 (연소득 × ${Math.round(np.livingRatio * 100)}% × a(${tInd}년, ${np.discount * 100}%))`, won(living0)],
          [`교육·결혼 자금 (${p.childrenAges.length}명 × ${won(np.educationPerChild)})`, won(p.childrenAges.length * np.educationPerChild)],
          ["정리자금", won(np.finalExpense)],
          ["현재 필요액 (기준액)", won(c[0])],
          [`막내 독립(${yr(p, tInd)}) 이후 필요액`, `${won(c[Math.min(n - 1, tInd)])} (${Math.round((c[Math.min(n - 1, tInd)] / c[0]) * 100)}%)`],
          ["기준보험금 제안 (필요액 − 유동자산·기존 보장)", won(roundS0(c[0] - p.liquidAssets - p.groupCover - p.termCover))],
        ],
      };
    }
    case "debt": {
      ...
      const pay = p.debtMethod === "annuity" && p.debtRate > 0 ? (p.debt * p.debtRate) / (1 - (1 + p.debtRate) ** -p.debtYears) : p.debt / p.debtYears;
      ...
      return {
        ...
        figures: [
          [`대출 ${won(p.debt)} · ${method} · 금리 ${(p.debtRate * 100).toFixed(1)}% · 만기 ${p.debtYears}년`, `연 상환액 약 ${won(Math.round(pay))}`],
          ...pts,
    ...
    case "retire": {
      ...
      const r = retireNeed({ age: p.age, retirementAge: p.retirementAge, hasSpouse: p.hasSpouse, spouseSex: p.sex === "M" ? "F" : "M", spouseAge: p.spouseAge, livingMonthly: p.livingMonthly, retireAssets: p.retireAssets, preNeed: base }, np, table);
      const tRet = Math.max(0, p.retirementAge - p.age);
      return {
        ...
        figures: [
          ...
          ...(p.hasSpouse ? [[`은퇴(${p.retirementAge}세) 시 배우자 나이 · 기대여명`, `${r.spouseAgeAtRetire}세 · ${r.expectancy.toFixed(1)}년`], [`배우자 생활비 현가 (월 ${won(p.livingMonthly)} × 12 × a(${Math.round(r.expectancy)}년))`, won(Math.round(r.living))]] as [string, string][] : [["배우자", "없음 → 정리자금만"] as [string, string]]),
    ...
    case "group": {
      ...
      return {
        ...
        figures: [
          [`필요액 (니즈${preNeed > 0 ? "" : " 없음 → 1억 가정"})`, won(base)], ["단체보험 보험금 (재직 중)", won(p.groupCover)],
          ["재직 중 개인 보장 비율 g = 1 − 단체 ÷ 필요액", `${Math.round(g * 100)}%${g < env.minMultiple ? ` (하한 ${env.minMultiple * 100}%)` : ""}`],
          [`퇴직(${p.retirementAge}세) 후`, "단체보험 소멸 → 100%"],
        ],
      };
    }
    case "estate": {
