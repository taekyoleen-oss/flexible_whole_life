...
export const PRESET_INFO: Record<PresetId, { control: "none" | "income" | "child" | "debt" | "retire"; rationale: string }> = {
  ...
  retire: {
    control: "retire",
    rationale: "은퇴 전후에는 소득이 끊기고 상속·정리 자금 수요가 커집니다. 은퇴 5년 전부터 매년 기준보험금의 10%씩 올려 은퇴 직전 해에 1.5배가 되고 그 뒤 유지합니다(그래프 편집과 같은 매년 한 칸 규칙). 은퇴시기는 55·60·65·70세 중 고르며 단체보험보완형과 같은 값을 씁니다(표준 65세).",
  },
  group: {
    control: "retire",
    rationale: "재직 중에는 단체보험이 사망을 일부 보장하므로 개인 보장은 50%면 됩니다. 퇴직(은퇴시기)으로 단체보험이 끝나기 5년 전부터 매년 기준보험금의 10%씩 올려 퇴직 직전 해에 100%가 되어 보장 공백을 막습니다(매년 한 칸 규칙). 은퇴시기는 은퇴증액형과 같은 값을 씁니다(표준 65세).",
  },
  estate: { control: "none", rationale: "50대 이후 자산이 늘면 상속세 재원도 커져야 합니다. 초기 5년은 50%로 시작해 매년 기준보험금의 10%씩 올려 15년 뒤 2배가 되며, 2배 또는 증액 종료 연령(70세)에서 고정합니다(매년 한 칸 규칙). 입력 정보와 무관한 표준 모양입니다." },
};
