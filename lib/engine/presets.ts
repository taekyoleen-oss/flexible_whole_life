...
function multiples(id: PresetId, c: PresetContext): number[] {
  ...
  switch (id) {
    ...
    case "retire": {
      const start = Math.max(FIX_YEARS, retire - c.age - 5);   // 은퇴 5년 전부터 +0.1/년, 은퇴 직전 해에 1.5
      for (let t = start; t < n; t++) S[t] = Math.min(1.5, 1 + 0.1 * (t - start + 1));
      return S;
    }
    case "group": {
      const start = Math.max(FIX_YEARS, groupEnd - c.age - 5);   // 퇴직 5년 전부터 +0.1/년, 퇴직 직전 해에 1.0
    ...
    case "estate": {
      for (let t = 0; t < n; t++) {
        ...
        const cand = Math.min(2, 0.5 + 0.1 * (t - FIX_YEARS + 1));   // +0.1/년, 15년 뒤 2배
