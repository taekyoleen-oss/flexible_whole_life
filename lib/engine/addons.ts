 ...
 * 감소형(자녀교육·대출상환)은 초기 고정 기간(fixYears, E01)은 정액, 그 뒤 만기(독립·상환 완료)까지 직선 감액. 정액: 기간 동안 같은 금액
 */
...
export function addonShape(a: Addon, independenceAge = 25, fixYears = 5): string {
  ...
  const years = a.kind === "education" ? Math.max(0, independenceAge - (a.childAge ?? independenceAge)) : a.years;
  const flat = Math.min(fixYears, years);
  return years > flat ? `1~${flat}년 정액, ${flat + 1}~${years}년 감액` : `1~${years}년 정액 후 0`;
}
